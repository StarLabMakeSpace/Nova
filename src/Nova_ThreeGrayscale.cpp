...
ThreeGrayscale::ThreeGrayscale(uint8_t port)
{
	switch(port)
	{
	    case C0:
...
void ThreeGrayscale::set_value(uint8_t a, uint8_t b, uint8_t c)
{
	//IICstart();
	...
	IICwrite(b);
	IICACK();
	IICwrite(c); // sends one byte
	IICACK();
	IICstop();
}

void ThreeGrayscale::value(uint8_t addr)
{
	uint8_t r[1];
    
    if (!readSTM8(addr, r, 1)) 
    {
        return;
    }
	date	   = r[0];
}
uint8_t ThreeGrayscale::readvalue(uint8_t num)
{
	value(num);
	return date;
}
uint8_t ThreeGrayscale::readSTM8(uint8_t address, uint8_t *buf, uint8_t count) {
...
void ThreeGrayscale::IICbegin(void)
{
...
bool ThreeGrayscale::IICstart(uint8_t addr)
{
	digitalWrite(SDA_pin, LOW);
	delayMicroseconds(I2C_DELAY_USEC);
...
void ThreeGrayscale::IICACK(void)
{
...
bool ThreeGrayscale::IICrestart(uint8_t addr)
{
	digitalWrite(SDA_pin, HIGH);
	digitalWrite(SCL_pin, HIGH);
	delayMicroseconds(I2C_DELAY_USEC);
...
uint8_t ThreeGrayscale::IICread(uint8_t last) {
  uint8_t b = 0;
 ...
 */
bool ThreeGrayscale::IICwrite(uint8_t data) {
  // write byte
  digitalWrite(SCL_pin, LOW);
